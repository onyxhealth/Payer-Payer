...
TRUST_FRAMEWORK = "http://hl7.org/fhir/us/ndh/StructureDefinition/base-ext-trustFramework"
...
def get_certificate(resource, organization_info={}, cert_type="mtls", verbose=False):
    #  "extension": [
    #    {
    ...
    #   ]
    if cert_type == "trust":
        cert_compare = TRUST_FRAMEWORK
    else:
        cert_compare = PUBLIC_CERT_EXTENSION
    if 'extension' in resource.keys():
        ...
        for e in resource['extension']:
            if e['url'] == cert_compare:
                for m in e['extension']:
                    if cert_type == "trust":
                        if m['url'] == "qualifier":
                            cert_info['certificateType'] = "Trust Framework Public Cert"
                        if m['url'] == "publicCertificate":
